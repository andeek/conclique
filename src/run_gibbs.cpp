...
arma::mat run_conclique_gibbs(List conclique_cover, List neighbors, arma::mat inits, Function conditional_sampler, List params, int n_iter = 100) {
  ...
  for(int i = 0; i < n_iter; ++i) { // iterations
    ...
    mat data_copy = data;
    data_copy.reshape(1, data.n_elem);
    result.row(i) = data_copy;
  } // end i
...
//' 
// [[Rcpp::export]]
arma::mat run_sequential_gibbs(List neighbors, arma::mat inits, Function conditional_sampler, List params, int n_iter = 100) {
  mat result(n_iter, inits.n_elem);
  mat data = inits;
  ...
  for(int i = 0; i < n_iter; ++i) { // iterations
    for(int j = 0; j < data.n_elem; ++j) { // each data point in sequence
      rows(0) = j;
      for(int n = 0; n < N; ++n) { // neighborhood structures
        mat neigh = neighbors[n];
        int q = neigh.n_cols - 1;
        uvec cols = regspace<uvec>(1,  1,  q);
        uvec idx = conv_to<uvec>::from(vectorise(neigh.submat(rows, cols)));
